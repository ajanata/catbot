...
//    final boolean useTls = Boolean.valueOf(catbot.getBotProperty(botId, PROP_TLS_ENABLED));
    ...
    for (int i = 0; i < numChannels; i++) {
      channels.add(catbot.getBotProperty(botId, PROP_CHANNELS + "." + i));
    }
    ...
    final Configuration config = new Configuration.Builder()
        ...
        .addAutoJoinChannels(this.channels)
        .addListener(this)
        .buildConfiguration();
  ...
  public void onMessage(MessageEvent event) throws Exception {
    ...
    if (text.startsWith(catbot.getBotProperty(botId, PROP_TRIGGER_PREFIX))) {
      ...
      LOG.trace(String.format("Message with trigger prefix from %s to %s: %s", fromName, event.getChannelSource(), text));
      ...
      if (parts.length > 0) {
        ...
        if (null != handler) {
          ...
          final String response = handler.handleMessage(botId, fromName, "", channel, String.join(" ", params));
          if (null != response) {
