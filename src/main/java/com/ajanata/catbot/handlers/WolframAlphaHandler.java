...
public class WolframAlphaHandler implements Handler {
  ...
  public void init() {
	...
	public String handleMessage(final int botId, final String fromName, final String fromId, final String chatId, final String message) {
      ...
      LOG.error(String.format("Unable to query Wolfram Alpha for %s in %s: %s", fromName, chatId, message), e);
      return String.format("I'm sorry, @%s, but I can't query Wolfram Alpha right now: %s", fromName, e.getMessage());
    }
	  ...
	  for (final WAPod pod : result.getPods()) {
	    ...
	    if (pod.isError()) {
	      ...
	      for (final WASubpod subpod : pod.getSubpods()) {
	        LOG.trace(String.format("Subpod (%s): %s", subpod.getClass().getSimpleName(), subpod.getTitle()));
	        for (final Object element : subpod.getContents()) {
	          LOG.trace(String.format("Element (%s): %s", element.getClass().getSimpleName(), element));
